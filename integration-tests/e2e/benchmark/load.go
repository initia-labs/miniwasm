...
// PreSignWasmExecTxs generates and signs all wasm execute transactions offline in parallel.
func PreSignWasmExecTxs(
	ctx context.Context, t *testing.T, cl *cluster.Cluster, cfg BenchConfig, metas map[string]cluster.AccountMeta,
	contractAddr, execMsg string, gasLimit uint64,
) []cluster.SignedTx {
	t.Helper()
	total := cfg.TotalTx()
	txs := make([]cluster.SignedTx, 0, total)
	...
	for _, name := range cl.AccountNames() {
		...
		go func() {
			...
			for i := 0; i < cfg.TxPerAccount; i++ {
				seq := meta.Sequence + uint64(i) //nolint:gosec // i is bounded by TxPerAccount
				signed, err := cl.GenerateSignedWasmExecTx(
					ctx, name, contractAddr, execMsg,
					meta.AccountNumber, seq, gasLimit,
				)
				if err != nil {
					t.Logf("[pre-sign-wasm] failed from=%s seq=%d err=%v", name, seq, err)
					continue
				}
				mu.Lock()
				txs = append(txs, signed)
	...
	wg.Wait()
	t.Logf("Pre-signed %d/%d wasm exec txs", len(txs), total)
...
// WasmExecSequentialLoad returns a LoadFn that submits wasm execute transactions
// sequentially per account, pinned to a single node.
func WasmExecSequentialLoad(contractAddr, execMsg string, gasLimit uint64) func(ctx context.Context, cl *cluster.Cluster, cfg BenchConfig, metas map[string]cluster.AccountMeta) LoadResult {
	return func(ctx context.Context, cl *cluster.Cluster, cfg BenchConfig, metas map[string]cluster.AccountMeta) LoadResult {
		if cfg.NodeCount <= 0 {
			panic("WasmExecSequentialLoad: cfg.NodeCount must be > 0")
		}
...
// WasmExecBurstLoad returns a LoadFn that submits wasm execute transactions concurrently.
func WasmExecBurstLoad(contractAddr, execMsg string, gasLimit uint64) func(ctx context.Context, cl *cluster.Cluster, cfg BenchConfig, metas map[string]cluster.AccountMeta) LoadResult {
	return func(ctx context.Context, cl *cluster.Cluster, cfg BenchConfig, metas map[string]cluster.AccountMeta) LoadResult {
		if cfg.NodeCount <= 0 {
			panic("WasmExecBurstLoad: cfg.NodeCount must be > 0")
		}
