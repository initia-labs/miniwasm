...
// StoreWasmContract stores a wasm contract and returns the tx result.
func (c *Cluster) StoreWasmContract(ctx context.Context, fromName, wasmPath string, viaNode int) TxResult {
	node, err := c.getNode(viaNode)
	if err != nil {
		return TxResult{Err: err}
	}
	...
	c.t.Logf("[wasm-store] from=%s code=%d txhash=%s", fromName, res.Code, res.TxHash)

	return res
}

// InstantiateWasmContract instantiates a wasm contract and returns the tx result.
func (c *Cluster) InstantiateWasmContract(ctx context.Context, fromName string, codeID, initMsg, label string, viaNode int) TxResult {
	node, err := c.getNode(viaNode)
	if err != nil {
		return TxResult{Err: err}
	}
	...
	c.t.Logf("[wasm-instantiate] from=%s codeID=%s code=%d txhash=%s", fromName, codeID, res.Code, res.TxHash)

	return res
}

// ExecuteWasmContract executes a wasm contract with explicit sequence/account number.
func (c *Cluster) ExecuteWasmContract(ctx context.Context, fromName, contractAddr, execMsg string, accountNumber, sequence, gasLimit uint64, viaNode int) TxResult {
	node, err := c.getNode(viaNode)
	if err != nil {
		return TxResult{Err: err}
	}
	...
	c.t.Logf("[wasm-execute] from=%s seq=%d gas=%d contract=%s code=%d txhash=%s",
		fromName, sequence, gasLimit, contractAddr, res.Code, res.TxHash)

	return res
}

// GenerateSignedWasmExecTx generates a signed, base64-encoded wasm execute transaction offline.
func (c *Cluster) GenerateSignedWasmExecTx(
	ctx context.Context,
	fromName, contractAddr, execMsg string,
...
// EstimateWasmGas estimates gas for a wasm execute call using --gas auto --generate-only.
func (c *Cluster) EstimateWasmGas(ctx context.Context, fromName, contractAddr, execMsg string, viaNode int) (uint64, error) {
	node, err := c.getNode(viaNode)
	if err != nil {
		return 0, err
	}
	...
	c.t.Logf("[wasm-estimate] from=%s contract=%s gas=%d", fromName, contractAddr, gas)

	return gas, nil
}

// ExtractCodeID extracts the code_id from a wasm store tx result's events.
func ExtractCodeID(txResult map[string]any) (string, error) {
	...
	return "", errors.New("code_id not found in tx events")
}
...
// ExtractContractAddress extracts the contract address from an instantiate tx result's events.
func ExtractContractAddress(txResult map[string]any) (string, error) {
	...
	for _, ev := range events {
		...
		for _, a := range attrs {
			...
			key, _ := attr["key"].(string)
			if key == "_contract_address" || key == "contract_address" || key == "contract-address" || key == "contract" {
				val, _ := attr["value"].(string)
				if val != "" {
					return val, nil
				}
...
func findRepoRoot() (string, error) {
	...
	for {
		if _, err := os.Stat(filepath.Join(current, "go.mod")); err == nil {
			...
			if readErr == nil && strings.Contains(string(modData), "github.com/initia-labs/miniwasm\n") {
				return current, nil
			}
