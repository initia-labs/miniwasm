...
func DeconstructDenom(ac address.Codec, denom string) (creator string, subdenom string, err error) {
	...
	creator = strParts[1]
	creatorAddr, err := ac.StringToBytes(creator)
	if err != nil {
	...
	strAddr, err := ac.BytesToString(creatorAddr)
	if err != nil {
		return "", "", errorsmod.Wrapf(ErrInvalidDenom, "Invalid creator address (%s)", err)
	}

	return strAddr, subdenom, nil
}
